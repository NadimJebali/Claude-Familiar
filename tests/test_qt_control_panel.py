...
_SETTINGS = {
    ...
    "usage_api_enabled": False, "theme": "classic", "context_window": "auto",
}
...
def test_save_persists_every_current_value(app, monkeypatch):
    ...
    panel._window_mode.setCurrentIndex(
        panel._window_mode.findData("1m"))                 # pin the gauge window (#95)
    panel._save()
    ...
    assert captured["context_window"] == "1m"
    assert set(captured) == {
        ...
        "usage_api_enabled", "theme", "context_window"}
    panel.close()
